...
public class SwiftyRSA: NSObject {
    ...
    public static var defaultDigest: DigestType = .SHA1
    ...
    */
    @nonobjc public func publicKeysFromString(_ inputPEMString:String) -> [SecKey] {
        ...
        guard let publicKeyRegexp = publicKeyRegexp, inputPEMString.count > 0 else {
            return response
        }
        ...
        for result in matches {
            ...
            if let key = try? self.publicKeyFromPEMString(String(thisKey)) {
                response.append(key)
            }
        }
